...
<div class="entries">
    <h1 class="proheader">Delete Form</h1>

    <form name="deleteform" action ="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype='multipart/form-data'>
        <center>
        <table class="tablee">
            <tr>
                <th>Product ID : </th>
    ...
    <?php
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["aproductid"])) {
        ...
        if (!empty($productid)) {
            ...
            $connect = mysqli_connect($host, $user, $passwd, $database) or die("Could not connect");

            // Query to check if product exists
            $myquery = "SELECT * FROM product WHERE Id = '$productid'";
            ...
            if (!$myrow) {
                echo "<p>No such record</p>";
            } else {
                // If record exists, delete it
                $sql = "DELETE FROM product WHERE Id = '$productid'";
                mysqli_query($connect, $sql);
                echo "<p>Product ID '$productid' has been deleted from the database.</p>";
            }
