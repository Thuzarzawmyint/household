...
footer {
        ...
        .error-message::before {
            content: "\26A0"; /* Exclamation symbol */
            margin-right: 10px;
            font-size: 1.5em;
            color: #dc3545;
        }
...
$pdname = $_POST["pdtname"];
$selprice = $_POST["sprice"];
...
if (in_array($imageFileType, $extensions_arr)) {
    ...
    if (($_FILES['pfile']['size'] >= $maxsize) || ($_FILES["pfile"]["size"] == 0)) {
        echo '<div class="message error-message">File too large. File must be less than 5MB.</div>';
    } else {
        if(move_uploaded_file($_FILES["pfile"]["tmp_name"], $target_file)) { 
            $imageData = file_get_contents($target_file);
            $imageData = mysqli_real_escape_string($connect, $imageData);
        // Update record
        $myquery = "UPDATE product SET catname='$catename', productname='$pdname', price='$selprice', photo='".$imageData."' WHERE Id='".$productid."'";
        mysqli_select_db($connect, $database);

        if (!mysqli_query($connect, $myquery)) {
            die('Error: ' . mysqli_error($connect));
        } else {
            echo '<div class="message success-message">Successfully updated!</div>';
        }
    }}
} else {
    echo '<div class="message error-message">Invalid file extension. Only PNG, JPEG, and JPG are allowed.</div>';
}}
?>
